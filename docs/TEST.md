...
| # | 操作 | 预期 |
| --- | --- | --- |
| 1 | 在菜单或设置区点击目标（非活跃）账号的切换 | 数秒内活跃标记移到目标账号；**全程无钥匙串授权弹窗**（设计 v1.5 ACL 预授权） |
...
#### `TC-004` 校验失败自动回滚（断网模拟） — 对应 `REQ-002` · 级别: 回归 · 执行者: 人工

前置：列表 ≥2 个有备份账号；记录当前活跃账号邮箱；断开网络（关 Wi-Fi）。

| # | 操作 | 预期 |
| --- | --- | --- |
| 1 | 点击切换到另一账号 | 出现 `ERR-SWITCH-VALIDATE` 提示，文案含「已回滚」语义 |
| 2 | 查看 Accounts 区活跃标记 | 仍在原账号 |
| 3 | 恢复网络后终端跑 `claude /status` | 显示原账号邮箱（凭据位未变） |
...
| # | 操作 | 预期 |
| --- | --- | --- |
| 1 | 打开 Claude 菜单 | 出现「Claude Accounts」小节，两个账号各一行，活跃账号带勾选标记 |
...
| # | 操作 | 预期 |
| --- | --- | --- |
| 1 | 查看两个合成账号行 | 均带「needs capture」标记，切换入口置灰不可点（`ERR-BACKUP-MISSING` 语义） |
| 2 | 打开 Claude 菜单账号小节查看合成账号行 | 行置灰不可点；`claude-accounts` 日志无 `switch` 事务开始行 |
